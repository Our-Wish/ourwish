...
def _best_by_payout(product, monthly, use_max):
    ...
    for option in product.options.all():
        ...
        payout = calculate_after_tax_payout(
            monthly, option.save_term, rate, option.intr_rate_type
        )
...
class FavoriteListCreateView(APIView):
    ...
    def get(self, request):
        ...
        for fav in favorites:
            ...
            # 예상 수령액은 조회 프로필(월저축액)이 있을 때만 계산. 없으면 null.
            expected_payout = None
            if profile is not None:
                best = _best_by_payout(product, profile.monthly_amount, use_max)
                if best is not None:
