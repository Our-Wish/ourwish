...
OurWish의 추천 로직은 **사용자 조회 프로필 기반의 필터링 + 세후 실수령액 정렬**입니다.
머신러닝 점수 모델 대신, "왜 이 순서로 추천되는지"를 설명하고 결과를 검증할 수 있도록 의도적으로 규칙 기반으로 설계했습니다.
...
- **기본(OR)**: 고른 조건 중 **하나라도** 충족하면 추천합니다. 우대 태그를 가진 상품이 데이터상 희소해, AND로 묶으면 결과가 0건이 되는 것을 방지합니다. 우대조건이 아예 없는(누구에게나 공정한) 상품도 함께 통과시킵니다.
...
- **환각 억제**: "원문에 근거가 있을 때만 태그를 켠다(추측·과잉판단 금지)"는 규칙과 "사원증=카드실적 아님" 같은 함정 예시를 프롬프트에 박아 오탐을 줄였습니다.
- 예시: `"가입금액 최저 1백만원이상"`(원문) → `min_limit = 1000000`(구조화)
- 추출된 태그·연령·최소금액이 그대로 추천 필터의 입력이 됩니다.

### ② 쉬운말 요약 + 용어 풀이

약관을 다 읽지 않아도 "이 상품이 어떤 분께 좋은지"를 한 줄로 알 수 있게, 상품 정보를 **추천 카피가 아니라 통역기처럼** 풀어 줍니다. 어려운 금융 용어(평잔·약정이율 등)는 따로 골라 쉬운 뜻을 붙입니다.
...
- **근거 한정(grounding)**: 그 상품 정보를 `developer` 메시지에 동적으로 주입하고, 정보에 없는 내용은 "제공된 정보에 없어요"라고 답하도록 가드레일을 설정해 할루시네이션을 억제합니다. 상품과 무관한 질문(예: "○○ 주가 알려줘")은 정중히 상품 관련 질문으로 되돌립니다.
